...
public class BrickTest extends JPanel implements KeyListener, ActionListener{
	...
	private static final int BOX_WIDTH = 800; // 전체 폭
	private static final int BOX_HEIGHT = 900; // 전체 높이
	private float ballRadius = 13; // 공의 반지름
	private float ballX = 130; // 공의 초기 X위치
	private float ballY = BOX_HEIGHT - ballRadius; // 공의 초기 Y위치
	...
	public void allocateBricks() {
		for(int i = 0; i < 10; i++) {
			if(i % 2 != 0) {
				brick[i] = new Rectangle(70, (365 + 60*i), 140, 20);
			}
			else {
				brick[i] = new Rectangle(300, (365 + 60*i), 140, 20);
			}
		}	
	}
